...
namespace DAL
{
    public class UserInfo
    {
        ...
        string cns = AppConfigutaionServices.configuration.GetConnectionString("cns");
        ...
        public int Update(Model.UserInfo user)
        {
            using(IDbConnection cn=new MySqlConnection(cns))
            {
                string sql = "update userinfo set password=@passWord,qq=@qq,email=@email,type=@type,userImg=@userImg);";
                return cn.Execute(sql, user);
            }
        }
